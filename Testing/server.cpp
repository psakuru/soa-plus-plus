...
#include "../SerializableObjects/ParticularSerializableObjects/Signal/SignalTypeConstants.h"
...
int main()
	...
	list<SerializableObject*>* outputList = riceviParametri(socket);
	list<SerializableObject*>::iterator i = inputList->begin();
	list<SerializableObject*>::iterator j = outputList->begin();
	for(; i != inputList->end(); i++)
		{
		cout << "Input parameter type: " << (*i)->getType() << endl;
		}
	for(; j != outputList->end(); j++)
	    {
	    cout << "Output parameter type: " << (*j)->getType() << endl;
	    }
	j = outputList->begin();
	RawByteBuffer* r = dynamic_cast<RawByteBuffer*>(*j);
